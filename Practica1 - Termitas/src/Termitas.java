...
public class Termitas extends PApplet {
    ...
    class ModeloTermitas{
      ...
      */
	  ...
	  int min = (direccion - 1) % 8; 
	  int max = (direccion + 1) % 8; 
	  int opcion = rnd.nextInt(3); //Un número aleatorio entre 0 y 2
	  if(opcion == 0)
	      return min;
	  else if (opcion == 1)
	      return direccion;
	...
	boolean hayAstilla(Termita t, int dir){
	    int nuevaPosX, nuevaPosY; // La posición en donde queremos saber si hay astilla
	    nuevaPosX = t.posX;
	  nuevaPosY = t.posY;
	  switch(dir){
	  case 0:  nuevaPosX = (t.posX-1)%ancho;
                   if(nuevaPosX < 0) nuevaPosX += ancho;
                   nuevaPosY = (t.posY-1)%alto;
                   if(nuevaPosY < 0) nuevaPosY += alto;
                   break;
          case 1:  nuevaPosY = (t.posY-1)%alto;
                   if(nuevaPosY < 0) nuevaPosY += alto;
                   break;
          case 2:  nuevaPosX = (t.posX+1)%ancho;
                   if(nuevaPosX < 0) nuevaPosX += ancho;
                   nuevaPosY = (t.posY-1)%alto;
                   if(nuevaPosY < 0) nuevaPosY += alto;
                   break;
          case 3:  nuevaPosX = (t.posX+1)%ancho;
                   if(nuevaPosX < 0) nuevaPosX += ancho;
                   break;
          case 4:  nuevaPosX = (t.posX+1)%ancho;
                   if(nuevaPosX < 0) nuevaPosX += ancho;
                   nuevaPosY = (t.posY+1)%alto;
                   if(nuevaPosY < 0) nuevaPosY += alto;
                   break;
          case 5:  nuevaPosY = (t.posY+1)%alto;
                   if(nuevaPosY < 0) nuevaPosY += alto;
                   break;
          case 6:  nuevaPosX = (t.posX-1)%ancho;
                   if(nuevaPosX < 0) nuevaPosX += ancho;
                   nuevaPosY = (t.posY+1)%alto;
                   if(nuevaPosY < 0) nuevaPosY += alto;
                   break;
          case 7:  nuevaPosX = (t.posX-1)%ancho;
                   if(nuevaPosX < 0) nuevaPosX += ancho;
                   break;
	  }
	  return mundo[nuevaPosY][nuevaPosX].estado;
	...
	void dejarAstilla(Termita t, int dir){
	    ...
	    t.cargando = false;
	    int nuevaDireccion = (dir + 4) % 8; /* La direción opuesta a dir */
      ...
      */
	  ...
	  mundo[t.posY][t.posX].estado = true;
	  t.cargando = false;
	  int posYRandom = rnd.nextInt(alto); //Nueva coordenada Y de la termita
	  int posXRandom = rnd.nextInt(ancho); //Nueva coordenada X de la termita
	  while(mundo[posYRandom][posXRandom].estado){
	      posYRandom = rnd.nextInt(alto);
	      posXRandom = rnd.nextInt(ancho);
	  }
	  t.posX = posXRandom;
	  t.posY = posYRandom;
