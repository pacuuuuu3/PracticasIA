...
public class Gatos extends PApplet {
    ...
    @Override
    public void setup(){
        anchoGato =  tamanioMosaico * 100;
        altoGato = tamanioMosaico * 100;
        anchoImagen = 100+30;
    ...
    class Gato{
        ...
        */
	    ...
	    LinkedList<Gato> sucesores = new LinkedList<>(); //La lista de sucesores que vamos a generar.
	    if(hayGanador)
		return sucesores;
	    for(int i = 0; i < 3; ++i){
		for(int j = 0; j < 3; ++j){
		    if(this.tablero[i][j] == 0){
			Gato sucesor = new Gato(this); //Copiamos el Gato para generar a sus sucesores.
			sucesor.tiraEn(i, j);
			sucesor.jugador1 = !sucesor.jugador1;
			sucesor.padre = this;
	...
	// dejar sin padre a los sucesores.
        ...
        boolean esSimetricoDiagonalInvertida(Gato otro){
            for(int i = 0; i < 3; i++){
                for(int j = 0; j < 3; j++){
                    if(tablero[j][i] != otro.tablero[i][j]) return false;
                }
            }
            return true;    
        }
        ...
        boolean esSimetricoDiagonal(Gato otro){
            return (tablero[0][0] == otro.tablero[2][2] && tablero[0][1] == otro.tablero[1][2] && tablero[1][0] == otro.tablero[2][1]
		    && tablero[0][2] == otro.tablero[0][2] && tablero[1][1] == otro.tablero[1][1] && tablero[2][0] == otro.tablero[2][0]
		    && tablero[2][2] == otro.tablero[0][0] && tablero[1][2] == otro.tablero[0][1] && tablero[2][1] == otro.tablero[1][0]);
        }
	    ...
	    for(int i = 0; i < 3; i++){
		for(int j = 0; j < 3; j++){
		    if(tablero[j][i] != otro.tablero[j][2-i]) return false;
                }
	    }
            return true;    
	    ...
	    for(int i = 0; i < 3; i++){
		for(int j = 0; j < 3; j++){
                    if(tablero[j][i] != otro.tablero[2-j][i]) return false;
                }
	    }
            return true;    
        }
	    ...
	    return (tablero[0][0] == otro.tablero[2][0] && tablero[0][1] == otro.tablero[1][0] && tablero[0][2] == otro.tablero[0][0]
		    && tablero[1][0] == otro.tablero[2][1] && tablero[1][1] == otro.tablero[1][1] && tablero[1][2] == otro.tablero[0][1]
		    && tablero[2][0] == otro.tablero[2][2] && tablero[2][1] == otro.tablero[1][2] && tablero[2][2] == otro.tablero[0][2]);
        }
        ...
        boolean esSimetrico180(Gato otro){
            for(int i = 0; i < 3; i++){
		for(int j = 0; j < 3; j++){
                    if(otro.tablero[j][i] != tablero[2-j][2-i]) return false;
                }
	    }
            return true;            
        }
        ...
        boolean esSimetrico270(Gato otro){
         return (tablero[0][0] == otro.tablero[0][2] && tablero[0][1] == otro.tablero[1][2] && tablero[0][2] == otro.tablero[2][2]
		    && tablero[1][0] == otro.tablero[0][1] && tablero[1][1] == otro.tablero[1][1] && tablero[1][2] == otro.tablero[2][1]
		    && tablero[2][0] == otro.tablero[0][0] && tablero[2][1] == otro.tablero[1][0] && tablero[2][2] == otro.tablero[2][0]);   
        }
